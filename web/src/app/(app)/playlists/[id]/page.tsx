...
import { RemoveEpisodeDialog } from '../components/remove-episode-dialog'
    ...
    <section className="space-y-6 p-6 pb-20">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Episódios de ruby com o robson da rocketseat.
        </p>
      </div>
        ...
        <TableBody>
          {Array.from({ length: 10 }).map((_, i) => (
            <TableRow key={i}>
              ...
              <TableCell>
                <RemoveEpisodeDialog episodeId={'4567'}>
                  <button className="inline-flex items-center justify-center whitespace-nowrap rounded-full h-8 w-8 ring-offset-background transition-colors hover:bg-muted-foreground/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:opacity-50 group">
                    <Trash className="h-4 w-4 stroke-muted-foreground group-hover:stroke-violet-700 group-hover:fill-violet-400 transition-colors" />
                  </button>
                </RemoveEpisodeDialog>
              </TableCell>
              <TableCell>
